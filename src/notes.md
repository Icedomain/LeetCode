...
* $dp(K, N)=1+\min _{1 \leq X \leq N}(\max (d p(K-1, X-1), d p(K, N-X)))$
* 二分查找
...
* 用与原矩阵同样大小的两个矩阵,一个为left(左边有多少个连续的1),一个up(上边有多少个连续的1)
* 从 min(up[i][j], left[i][j])到1反向遍历边长k
* 左边上方up[i][j-k+1] >= k和上方左边left[i-k+1][j] >= k就可以满足是1围绕的正方形
