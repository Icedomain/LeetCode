...
胡欣毅 邮箱:[icedomain\_hu@qq.com](icedomain_hu@qq.com)
...
* 最后都是1 前面的相加 再加1 补0; 最后都是0 补0: 最后一个1 一个0 补1
...
* 三个分别对应于加、减、替换
* dp[i-1][j]+1, dp[i][j-1]+1, dp[i-1][j-1]+(word1[i-1]!=word2[j-1]) 三种情况取小

### 73、矩阵置零

* 直接查找是0的位置,然后对每个位置行列各置为零
* 第一行第一列做标记,行:从上倒下,列:**从右往左**置0.**记得回来处理一下第一行,全部置成0**
...
* 从左往右遍历s串
  ...
  - 当字典的值大于0时,寻找才是合法的,计数器加一
  - 当计数器等于t串长度,表示搜到了一个区间,若比之前的短,更新
  - 此时还需要将区间左侧右移.(若左侧在字典里,右移的时候需要字典对应值加一,计数器减一更新)
...
[牛逼了](https://www.cnblogs.com/lichen782/p/leetcode_Largest_Rectangle_in_Histogram.html)
...
* f[i][j]表示s1[0,i]和s2[0,j]匹配s3[0, i+j]
...
* 给子树一个限定的值域范围
* 缩小子树的时候变化值域范围,如左子树($-\infty,root.val$)
...
* 根相同
* 左子树右子数的根相同并左子树的左子树～右子树的右子树、左子树的右子树～右子树的左子树

### 102、二叉树的层次遍历

* 定义递归函数fun(root,level,result)
* 如果递归的层加深了,result加入空列表[]
...
- 方法一
  * 利用栈的思想,从上倒下扫描,装入stack,这样尾部就是我们新加入的
  * 在stack的尾部的节点list,向下新一轮扫描并压入栈
  * 上一步的节点list的val插入到头部达到倒序
- 方法二
  * 102、二叉树的层次遍历加一个反转
...
* 左右子树都空(最下面的),并sum == root.val的时候该路径满足,路径添加到result
...
* 线性扫描,如果右侧的最大元素减去当前元素的值大于目前的profit,就把profit刷新
* 描到第i个元素时,如果在这一天卖股票,盈利就是dp[i] = prices[i] - minelement
...
* 递归 
* return的时候只需要一个方向的值,因为在递归过程中,子树只有向父节点返回(L->root或R->root),不存在L->root->R
...
* 单独的回文判断函数
* 类似path和result的思路,这里遍历的是(index,end)
...
* ==从右往左== , $f(i)=\min\{f(j+1)+1\}$,$[i,j]$之间是一个回文串
...
* 动态规划
* $f(i)$表示$s[0,i]$是否可以拆分
* $f(i) = 任意一个j : (f(j) \quad\&\&\quad s[j+1,i] \in dict), 0 \leq j < i$
...
* nums[0:-1]和nums[len(nums) != 1:]取大即可
* 两个指针,一个now、一个prev
...
* 先扣出来2的部分
* 当前要么走要么不走( **dp[i] = min(dp[i], dp[i- j\*\*2] + 1)**)
...
* 收敛法 $ x = \frac{1}{2}(x+\frac{num}{x}) $ 收敛下去就是$\sqrt{num}$
